/*
  ...
  Flujo:
  - Espera botón
  - Al presionar botón manda START inmediatamente
  - Espera 7 segundos sin mandar datos de velocidad
  - Manda encabezado CSV
  - Durante 60 segundos manda velocidad a 10 Hz
  - Al terminar manda END

  Salida durante prueba:
  START
...
// Espera entre botón y comienzo real de datos
const uint32_t START_DELAY_MS = 7000;

// Duración real de toma de datos
...
// =====================================================
// VARIABLES DE SENSOR Y VELOCIDAD
// =====================================================
...
void startWaitingPhase() {
  ...
  // START se manda de inmediato para que la Raspberry cambie a game.mp4.
  sendLine("START");

  Serial.print("# Esperando ");
  Serial.print(START_DELAY_MS / 1000);
  Serial.println(" segundos antes de iniciar datos...");
}

void startActivePhase() {
  testState = TEST_ACTIVE;
  activeStartMs = millis();

  // Reiniciamos otra vez justo al inicio real de medición.
  // Así los 7 segundos de espera no contaminan velocidad ni suavizado.
  resetSpeedState();

  sendCsvHeader();

  Serial.println("# Iniciando envio de datos por 60 segundos.");
}
...
void updateTestState() {
  ...
  if (testState == TEST_WAITING) {
    if ((nowMs - waitStartMs) >= START_DELAY_MS) {
      startActivePhase();
    }
    return;
  }

  if (testState == TEST_ACTIVE) {
    if ((nowMs - activeStartMs) >= TEST_DURATION_MS) {
      stopTest();
    }
    return;
  }
...
void setup() {
  ...
  Serial.print("# Espera antes de datos: ");
  Serial.print(START_DELAY_MS / 1000);
  Serial.println(" segundos.");

  Serial.print("# Duracion de datos: ");
